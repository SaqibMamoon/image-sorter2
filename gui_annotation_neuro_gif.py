...
def order_properly(l):
    ...
    for typ in ['HGG','LGG']:
        ...
        l_typ = sorted(l_typ)
        l_typ = [typ+str(k)+'_.gif' for k in l_typ]
        output += l_typ
    return output
...
class ImageGui:
    ...
    def __init__(self, master, labels, keys, paths):
        ...
        for label in labels:
            self.buttons.append(
                    tk.Button(frame, text=label, width=10, height=2, fg='green', bg='black', command=lambda l=label: self.vote(l))
            )
    ...
    def set_image(self, path):
        ...
        """
        #pass
        self.image_panel = GIFLabel(self.frame, path)
        self.image_panel.grid(row=2, column=0, columnspan=self.n_labels+1, sticky='we')
...
#         if file.endswith(".tif") or file.endswith(".gif"):
